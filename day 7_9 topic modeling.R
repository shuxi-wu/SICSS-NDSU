...
docvars(tweetCorpus, field = "month") <- tweets$month # covariance field month; pass the month variable to the tweetcorpus as month; this creates a new var called month in tweetcorpus
...
topfeatures(tweetDfm, n = 100, scheme = "docfreq") #shows the top words; can use this to fine-tune data/do more cleaning

#prepare stm data
tweets.stm <- convert(tweetDfm, to = "stm") #can replace stm with other packages like ctm etc.; this transform the dfm into stm data
...
#1.Statistical fit: look at the highest exclusivity and semantic coherence scores!
#2.Interpretability and relevance of topics

#Let's find the optimal K, this will take a long time, be prepared; k is the number of topics; here we're asking for 3-10 topics 
...
#both exclusivity coefficient and semantic coherence coefficient are high (this is where you need to choose)

#Now, let's run the 5-topic model  
model <- stm(out$documents, out$vocab,
             K = 5, 
             max.em.its = 100, #its number should follow searchK output; this is the number of iterations wanted; may want a higher number if output says terminated before convergence reached
             data = out$meta, 
             init.type = "Spectral")
...
labels <- labelTopics(model, n = 20) # this gets top 20 keywords, but you can get more 
...
plot(model, type = c("summary"),
     ...
     topic.names = c("Topic 1: graduation",
                     ...
                     "Topic 3: atmosphere",
                     "Topic 4: events",
                     "Topic 5: prospective student information"),
...
plot(model, 
     type="perspectives", 
     topics=c(3,5), 
     plabels = c("Topic 3: atmosphere","Topic 5: prospective info"))
...
#words used in topics
labelTopics(model, c(1:6), frexweight = 0.5,n = 15) 

#Let's focus on FREX
#It shows words that are comparatively common and exclusive for a topic compared to others
labels <- labelTopics(model, n = 20) # this gets top 20 keywords, but you can get more 
topwords <- data.frame("features" = t(labels$frex))
colnames(topwords) <- paste("Topics", c(1:6))
topwords[1:6]
...
plot(model, type = c("summary"),
     labeltype = c("frex"),
     topic.names = c("Topic 1: pride",
                     "Topic 2: graduation",
                     "Topic 3: learning experience",
                     "Topic 4: covid",
                     "Topic 5: academic resources", 
                     "Topic 6: school strength"),
     main = c("Topic distribution"),xlim = c(0, 0.5),
     custom.labels="")
...
result1 <- estimateEffect(1:6 ~ university, model,
                          meta = out$meta, uncertainty = "Global")
topiceffect1<-summary(result1, topics = c(1:6))
topiceffect1
# a positive coeff on UofND in topic 1 means UofND tweeted more on topic 1
...
plot(x = result1, 
     ...
     xlim = c(0, 0.5))
# shows expected proportion that the university tweeted in this topic 

#month as covariate
result2 <- estimateEffect(1:6 ~ month, model,
                          meta = out$meta, uncertainty = "Global")
topiceffect2<-summary(result2, topics = c(1:6))
topiceffect2
...
result3 <- estimateEffect(1:6 ~ month + university + month*university, model,
                          meta = out$meta, uncertainty = "Global")
topiceffect3<-summary(result3, topics = c(1:6))
topiceffect3
