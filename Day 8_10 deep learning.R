...
model %>%
  layer_flatten(input_shape = c(28, 28)) %>% # from 2D to 1D input
  layer_dense(units = 128, activation = 'relu') %>% # collect features that you think is useful; activation means what method is used to find the threshold to fire
  layer_dense(units = 10, activation = 'softmax') # The last layer should have a unit equal to # of classes
...
model %>% compile(
  optimizer = 'adam', 
  loss = 'sparse_categorical_crossentropy',  # i.e. the method of minimizing loss/error; "Loss is the penalty for a bad prediction." https://developers.google.com/machine-learning/crash-course/descending-into-ml/training-and-loss
  metrics = c('accuracy')
)
# now train the model!
model %>% fit(train_images, train_labels, epochs = 5, verbose = 2) # Run twice. Compare results.
# If you use a CNN, add more epochs.
# now test the model; how did we do?
